...
def findDatabase(ssh, path='/'):
	databases = []
	sizes = []
	_, ssh_stdout, _ = ssh.exec_command('for x in $(find '+path+' -type f \\( -name *.db -o -name *.sqlite3 -o -name *.sqlite -o -name *.plist -o -name *.jpg -o -name *.jpeg -o -name *.dat -o -iname \"*.realm\" \\) 2>/dev/null); do echo "$x";done')
	for x in ssh_stdout.read().decode().replace('Application\n', 'Application ').strip('\n').split('\n'):
		_, ssh_stdout, _ = ssh.exec_command('md5sum \"'+x+'\"')
